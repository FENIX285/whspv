...
function AudioPlayer({ buffer, mimeType, isMe, isPreview }: { buffer: ArrayBuffer, mimeType: string, isMe: boolean, isPreview?: boolean }) {
  ...
  const [volumeBoost, setVolumeBoost] = useState<number>(1.5);
  ...
  const cycleVolume = (e: React.MouseEvent) => {
    e.stopPropagation();
    setVolumeBoost(prev => {
      if (prev >= 2.4) return 1.0;
      if (prev >= 1.9) return 2.5;
      if (prev >= 1.4) return 2.0;
      return 1.5;
    });
  };
  ...
  return (
    <div className={`flex items-center gap-3 w-52 ${isMe ? 'text-emerald-100' : 'text-slate-200'}`}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center justify-between text-[10px] opacity-80 mt-1 font-mono">
          <button
            type="button"
            onClick={cycleVolume}
            className={`px-1.5 py-0.5 rounded flex items-center gap-0.5 font-bold transition-all ${
              volumeBoost > 1.0 
                ? (isMe ? 'bg-emerald-400/30 text-emerald-100' : 'bg-emerald-500/20 text-emerald-400')
                : 'text-slate-400 opacity-60 hover:opacity-100'
            }`}
            title="Click to boost volume level (1.0x - 2.5x)"
          >
            <Volume2 className="w-3 h-3" />
            <span>{volumeBoost}x</span>
          </button>
          {!isPreview && (
            <span>{isPlaying || currentTime > 0 ? formatAudioTime(currentTime) : formatAudioTime(duration)}</span>
          )}
        </div>
      </div>
      <audio 
